...
if plotopt
    ...
    [nothing, cont] = contourf(dTs, phis, Emap); 
%     uistack(cont, 'bottom')
    box on
    axis on
...
%     set(gca,'Ytick',[1:30/dphi:length(phis)],'YtickLabel',phis(1:30/dphi:end))
    set(gca,'FontSize',12)
%     plot(it,ip,'|','Linewidth',3)
    plot([dTs(it), dTs(it)], [min(phis), max(phis)], 'k', 'linewidth', 1)
    plot([min(dTs), max(dTs)], [phis(ip), phis(ip)], 'k', 'linewidth', 1)
    xlabel('Splitting time (s)')
    ylabel('Fast polarization')
    set(gca, 'fontsize', 15); 
    mymap = [linspace(1, 0); linspace(1, .2); linspace(1, 1)]' 
    colormap(mymap); 
    h = colorbar()
    set(get(h,'label'),'string','Transverse energy');
    set(h, 'fontsize', 15); 
    if false; 
        saveas(gcf, 'figs/transverse_energy.png')
    end
end
