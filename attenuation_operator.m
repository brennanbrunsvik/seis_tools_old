function [ Dwt ] = attenuation_operator( Q0,c0,L,w,alpha,opt,w0 )
%[ Dwt ] = attenuation_operator( Q,L,omega,alpha,opt,w0 )
...
%             minimum phase 'mph'
%  w0      -  reference angular frequency (default is 2 pi = 1Hz)
% 
% OUTPUT:
...
% Dwt = exp( -0.5*abs(w)*L.*qinv_w./c0 ) .* exp( -sign(w).*1i.*w.*L.*(1./c_w - 1./c_inf) );
